...
class GraphTheory(Scene):
    def construct(self):
        ...
        graph = Graph(list(node_positions.keys()), edges, layout=node_positions, labels=True).scale(0.5)
        self.play(Create(graph))
        self.wait(1)
        t1 = Text("What is this?", font="Lucida Console", font_size=38).shift(DOWN * 2)
        self.play(graph.animate.to_edge(UP, buff=0.5))
        self.play(Write(t1))
        self.play(t1.animate.next_to(graph, buff=0.5))
        t2 = MarkupText(
            f"""
